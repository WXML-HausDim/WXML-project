...
# centers = np.array([(phi(thetas[0]-theta_tilde/2) + phi(thetas[0]+theta_tilde/2))/2, \
#                     (phi(thetas[1]-theta_tilde/2) + phi(thetas[1]+theta_tilde/2))/2, \
#                     (phi(thetas[2]-theta_tilde/2) + phi(thetas[2]+theta_tilde/2))/2])
# radii = np.array([abs(phi(thetas[0]-theta_tilde/2) - phi(thetas[0]+theta_tilde/2))/2, \
#                   abs(phi(thetas[1]-theta_tilde/2) - phi(thetas[1]+theta_tilde/2))/2, \
#                   abs(phi(thetas[2]-theta_tilde/2) - phi(thetas[2]+theta_tilde/2))/2])
...
width = 10 # must be even, and assumes x-axis symmetric about 0
height = 3
...
for i in range(0, width-1):
    ...
    turt.write(i+1 - width/2, align = 'center', font = ('Arial',12,'normal'))
...
colors = ['red', 'blue', 'green', 'pink', 'green'] # refinements <= 4
